...
var color256 = require('./color256');


function getColor(name) {
    var color = color256.get(name);
    if (color) {
...
function DocumentConversion(options, comments) {
    ...
    function convertRun(run, messages, options) {
        ...
        if (_stylePreservations.useBackgroundColorSpans && run.backgroundColor) {
            paths.push(htmlPaths.element('span', {style: 'background-color:' + getColor(run.backgroundColor)}, {fresh: false}));
        }
        if (_stylePreservations.useColorSpans && run.color) {
            paths.push(htmlPaths.element('span', {style: 'color:#' + run.color}, {fresh: false}));
        }
        if (_stylePreservations.useFontSizeSpans && run.fontSize) {
            var fontSize = _stylePreservations.useStrictFontSize ? run.fontSize / 2 : run.fontSize;
    ...
    function convertTable(element, messages, options) {
        var htmlPath = findHtmlPath(element, defaultTablePath);
            ...
            if (!_stylePreservations.ignoreTableElementBorders) {
                if (htmlPath._elements && htmlPath._elements.length > 0) {
                    if (element.preservedStyles) {
                        htmlPath._elements[0].attributes.style = preserverUtils.convertPreservableStylesToCssString(element.preservedStyles, _stylePreservations.reduceCellBorderStylesUsed);
                    }
                }
            }
        }
    ...
    function convertTableCell(element, messages, options) {
        ...
        if (_stylePreservations.applyTableStyles && element.preservedStyles) {
            ...
            attributes.style = preserverUtils.convertPreservableStylesToCssString(element.preservedStyles, _stylePreservations.reduceCellBorderStylesUsed);
        }
